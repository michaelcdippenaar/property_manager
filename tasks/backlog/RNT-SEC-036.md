...
## Acceptance criteria
- [ ] `MaintenanceRequestViewSet.get_permissions()` adds `create` to the agent-only guard, OR adds a separate `IsTenantOrAgent` permission class that returns 403 for suppliers
- [ ] `POST /api/v1/maintenance/` by a supplier user returns 403
- [ ] `TestMaintenanceRBAC::test_supplier_create_maintenance_is_documented` updated to assert strict `== 403` (not lenient `in (201, 400, 403)`)
- [ ] Tenant `create` flow still returns 201; agent `create` flow still returns 201
- [ ] POPIA note in view docstring: suppliers may only interact with jobs dispatched to them
