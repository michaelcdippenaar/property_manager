...
## Acceptance criteria
- [ ] `test_register_default_role_is_admin` and `test_first_registered_user_is_admin` updated to reflect current role names (`owner`/`agency_admin`)
- [ ] Rate-limit tests in `test_rate_limits.py::TestPublicSignMinuteThrottle` passing — URL routing fixed or tests updated to current URL pattern
- [ ] All 7 `test_municipal_bill_view.py` failures fixed — mock/tool_use block and error string mismatches resolved
- [ ] 2 tenant portal conversation test failures fixed — stale mock setups corrected
- [ ] 2 `TestGetAccessiblePropertyIds` failures fixed — `Mock not iterable` setup corrected
- [ ] `pytest` run exits with zero failures (or any intentionally skipped tests marked `xfail` with a comment)
