...
## Acceptance criteria
- [x] `POST /api/v1/esigning/submissions/` with a valid create payload returns 201 (not 422)
- [x] Root cause of 422 identified — likely a serializer validation change or a required field added without updating test fixtures
- [x] All 8 previously-failing tests pass:
...
- [x] Full esigning integration suite passes with no regressions
...
(2026-04-23 rentals-pm) Dedup sweep: test_esigning.py 36/36 passed, test_esigning_full.py 25/25 passed. All 8 named tests green. Moved to done.
