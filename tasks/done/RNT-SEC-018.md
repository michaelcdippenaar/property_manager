...
Duplicate of RNT-SEC-026 + RNT-SEC-033, both already shipped. `get_client_ip()` utility in `utils/http.py` centralises all XFF extraction; all three original sites (legal/serializers, esigning/audit, accounts/audit) migrated in RNT-SEC-026; three residual sites (accounts/views, accounts/serializers, esigning/views) migrated in RNT-SEC-033. 11 utility unit tests + full suite pass. Closing as duplicate.
