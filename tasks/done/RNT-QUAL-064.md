...
### Test run 2026-04-24 (tester)

1. `pytest config/tests/test_security_headers.py -v --no-cov` — 56/56 PASS, incl. all 4 TestProductionNoLocalhostOrigins tests.
2. `base.py` assertion — PASS: zero localhost/127.x entries in CORS_ALLOWED_ORIGINS and CSRF_TRUSTED_ORIGINS.
3. `local.py` check — PASS: localhost entries present (ports 5173, 5174, 5178, 9000, capacitor://localhost confirmed).
