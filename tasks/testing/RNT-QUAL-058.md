...
## Acceptance criteria
- [x] Both parsers reset `lastSlug` on encountering a two-space-indented non-comment line (feature block boundary), OR the simpler test-file version has an explicit comment explaining why it is safe without that reset
- [x] Either extract to a shared helper, or document the intentional difference
- [x] Existing drift CI check continues to pass
...
2026-04-24 — Extracted `FEATURE_BLOCK_BOUNDARY_RE = /^  \w[\w-]*:/` as a named constant into both files with a cross-reference comment. Added the missing boundary-reset branch to the test's parser loop (`if (FEATURE_BLOCK_BOUNDARY_RE.test(line)) lastSlug = null`). Replaced the script's more convoluted three-condition guard with the same single-regex check. All 3 Vitest drift tests pass; script exits 0 (21 slugs in sync).
