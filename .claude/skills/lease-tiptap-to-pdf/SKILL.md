...
description: >
  ...
  xhtml2pdf configuration, print CSS for leases, or troubleshooting the lease PDF output.
  Also trigger for: "generate PDF", "lease PDF", "PDF export", "PDF not rendering", "signed PDF",
  "audit trail", "xhtml2pdf", "pisa", "merge fields not filling", "PDF layout", "page breaks in PDF",
  "signature in PDF", "download lease", "PDF styling", or any question about how TipTap content
  becomes a downloadable/signable PDF. Even if the user just says "the PDF looks wrong" or
  "merge fields aren't being replaced" — use this skill.
---
...
three main stages: template rendering, signing, and PDF generation.
...
generate_lease_html(lease)          ← fills merge fields, maps signing roles
  ↓
  ├─→ DocuSeal path: upload_html_template() → create_submission()
  │     DocuSeal renders PDF and handles signing
  │
  └─→ Native path: create_native_submission()
        Stores HTML snapshot → signers sign via custom Vue UI
        ↓
        generate_signed_pdf(submission)   ← embeds signatures, renders xhtml2pdf
        ↓
...
**Location:** `backend/apps/esigning/services.py:386`
...
4. **Convert legacy signing spans** — old templates used `<span data-type="signature-block">`;
   converts to DocuSeal-native tags: `<signature-field>`, `<initials-field>`, `<date-field>`
5. **Map signer roles** — converts TipTap roles (landlord, tenant_1) to DocuSeal roles
   (First Party, Signer 2) unless `native=True`
...
When generating HTML for native signing (no DocuSeal), unfilled merge fields are preserved
as TipTap `<span data-type="merge-field">` nodes so the signing UI can render them as
editable inputs for the signer to fill in.

### build_lease_context()

**Location:** `backend/apps/esigning/services.py:166`
...
**Location:** `backend/apps/esigning/services.py:619`
...
**Location:** `backend/apps/esigning/services.py:930`

Takes a completed `ESigningSubmission` and produces final PDF bytes using `xhtml2pdf` (pisa).

### Processing steps:
1. **Strip PaginationPlus wrappers** — removes `page-break-after:always` divs that were used
   for TipTap's visual pagination; lets xhtml2pdf flow content naturally with `@page` rules
...
8. **Render with xhtml2pdf** — `pisa.CreatePDF(html, dest=buffer)` → PDF bytes

### xhtml2pdf Considerations

xhtml2pdf is lightweight but has limitations compared to WeasyPrint or wkhtmltopdf:
- Supports inline CSS and `@page` rules
...
- Tables render well but complex nested tables can break
- `page-break-before/after` CSS properties work for manual page breaks
- Font support is limited to standard web fonts (Arial, Times, Courier, etc.)
...
- TipTap's PaginationPlus uses `page-break-after:always` wrapper divs
- `generate_signed_pdf()` strips these wrappers because xhtml2pdf handles pagination via `@page`
- If page breaks are wrong, check that the regex stripping is matching correctly
...
### PDF layout issues
- xhtml2pdf doesn't support CSS flexbox or grid — use tables for layout
- Wide tables may overflow — set `width:100%` and use `word-break:break-all` for long text
