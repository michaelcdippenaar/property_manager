    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="p in properties" :key="p.id" class="card p-5 space-y-3">
        ...
        <div class="grid grid-cols-3 gap-2 text-center text-sm">
          ...
          <div><div class="text-lg font-bold text-green-600">{{ p.occupied_units }}</div><div class="text-micro text-gray-400">Occupied</div></div>
          <div><div class="text-lg font-bold text-amber-600">{{ p.active_issues }}</div><div class="text-micro text-gray-400">Issues</div></div>
        </div>
      </div>
