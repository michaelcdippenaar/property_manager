    ...
    <template v-else-if="submission">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <span v-if="submission.status === 'completed'" class="inline-flex items-center gap-1.5 text-sm font-medium text-success-700">
            <CheckCircle2 :size="15" />
            Mandate fully signed
          </span>
          <span v-else-if="submission.status === 'declined'" class="inline-flex items-center gap-1.5 text-sm font-medium text-danger-600">
            <XCircle :size="15" />
            Signing declined
          </span>
        ...
        >
          ...
          <div class="min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span class="text-micro px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500 capitalize">{{ signer.role }}</span>
            </div>
...
function signerDotClass(s: any) {
  if (isSignerDone(s))     return 'border-success-400 bg-success-50'
...
function signerNarrativeColor(s: any) {
  if (isSignerDone(s))     return 'text-success-600'
  if (isSignerDeclined(s)) return 'text-danger-500'
  if (isSignerViewing(s))  return 'text-info-600'
