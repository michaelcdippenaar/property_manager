<template>
  <div class="space-y-5">
    <!-- Loading -->
    <div v-if="loading" class="card p-6 space-y-3 animate-pulse">
    ...
    <!-- Not found -->
    <div v-else-if="!issue" class="text-center text-gray-400 py-16">
      Issue not found
    </div>
    ...
    <template v-else>
      ...
      <div class="flex items-start gap-3">
        <RouterLink to="/maintenance/issues" class="mt-1.5 p-1 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100 transition-colors flex-shrink-0">
          <ArrowLeft :size="18" />
        </RouterLink>
        <div class="min-w-0 flex-1">
          <div class="text-xs font-mono text-gray-500">{{ issue.ticket_reference || `#${issue.id}` }}</div>
          <h1 class="text-lg font-semibold text-gray-900 mt-0.5">{{ issue.title }}</h1>
        </div>
        <span :class="priorityBadge(issue.priority)" class="flex-shrink-0">{{ issue.priority }}</span>
      </div>
...
import api from '../../api'
import { ArrowLeft, Send, Loader2, MapPin } from 'lucide-vue-next'
