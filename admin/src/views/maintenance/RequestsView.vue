      ...
      <div v-for="req in requests" :key="req.id" class="card p-5 space-y-3">
        ...
        <div class="pt-2 border-t border-gray-100">
          <select
            ...
            class="w-full text-xs border border-gray-200 rounded-lg px-2 py-1.5 text-gray-600 bg-white outline-none focus:ring-1 focus:ring-navy/30 cursor-pointer"
          >
          ...
          <div v-if="req.supplier_name" class="flex items-center gap-1.5 mt-1.5 text-xs text-gray-500">
            <Truck :size="11" />
            {{ req.supplier_name }}
          </div>
        </div>
...
import api from '../../api'
import { Clock, Truck } from 'lucide-vue-next'
