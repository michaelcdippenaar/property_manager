"""
Phase 4 — Backfill orphan rows in the popia app to Klikk agency #1.
...
POPIA_MODELS = ["DSARRequest", "ExportJob"]
...
def backfill(apps, schema_editor):
    ...
    if klikk is None:
        print("[popia.0005] no Klikk agency — skipping")
        return

    counts: dict[str, int] = {}
    skipped: list[str] = []
    for label in POPIA_MODELS:
        try:
            Model = apps.get_model("popia", label)
        except LookupError:
    ...
    print(f"[popia.0005] backfilled to agency #{klikk.pk}: {counts}")
    if skipped:
        print(f"[popia.0005] skipped: {skipped}")
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("popia", "0004_backfill_agency_id_popia"),
