"""
Phase 4 — Backfill orphan rows in the audit app to Klikk agency #1.
...
def backfill(apps, schema_editor):
    ...
    if klikk is None:
        print("[audit.0006] no Klikk agency — skipping")
        return

    counts: dict[str, int] = {}
    skipped: list[str] = []
    for label in AUDIT_MODELS:
        try:
            Model = apps.get_model("audit", label)
        except LookupError:
    ...
    print(f"[audit.0006] backfilled to agency #{klikk.pk}: {counts}")
    if skipped:
        print(f"[audit.0006] skipped: {skipped}")
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("audit", "0005_backfill_agency_id_audit"),
