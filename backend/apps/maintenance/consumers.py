...
class MaintenanceActivityConsumer(AsyncWebsocketConsumer):
    ...
    def _call_agent_ai(self, message: str) -> str | None:
        ...
        system = (
            ...
            "IMPORTANT: When the tenant reports a maintenance issue, ask them to take and send a photo "
            "of the problem if it would help diagnose the issue (e.g. leaks, damage, mould, broken items). "
            "Say something like 'Could you take a photo of the issue? It will help us assess it faster.'\n"
            "Only ask for a photo once — do not keep requesting if they have already sent one or declined.\n\n"
            "RESOLUTION DETECTION: If the tenant says the issue is fixed, resolved, no longer a problem, "
            "or thanks you for the help indicating the matter is settled, ask them: "
            "'It sounds like this issue has been resolved. Would you like to close this ticket?' "
            "Only suggest closing once per conversation. Do not close the ticket yourself.\n\n"
