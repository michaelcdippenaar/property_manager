"""
Phase 4 — Backfill orphan rows in the contact app to Klikk agency #1.
...
def backfill(apps, schema_editor):
    ...
    if klikk is None:
        print("[contact.0004] no Klikk agency — skipping")
        return

    counts: dict[str, int] = {}
    skipped: list[str] = []
    for label in CONTACT_MODELS:
        try:
            Model = apps.get_model("contact", label)
        except LookupError:
    ...
    print(f"[contact.0004] backfilled to agency #{klikk.pk}: {counts}")
    if skipped:
        print(f"[contact.0004] skipped: {skipped}")
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("contact", "0003_backfill_agency_id_contact"),
