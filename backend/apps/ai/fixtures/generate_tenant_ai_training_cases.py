#!/usr/bin/env python3
"""Regenerate tenant_ai_training_cases.json. Run from repo: python backend/apps/ai/fixtures/generate_tenant_ai_training_cases.py"""
...
def main() -> None:
    ...
    # --- Property information (estate / unit facts — no repair ticket) ---
    t(
        "prop_info_pool_hours",
        "Communal pool hours.",
        "Tenant: What time does the estate pool close in summer?\nAssistant: Check house rules or the notice at the pool gate.",
        ["property_info", "faq"],
        chat_raw=j(
            {
                "reply": "Pool hours are in your house rules or on signage at the pool. Ask the estate office if unsure.",
                "conversation_title": "Pool hours",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={
            "json_ok": True,
            "maintenance_ticket_null": True,
            "conversation_title_non_null": True,
        },
        expected_chat_json={"maintenance_ticket_null": True},
    )
    t(
        "prop_info_gym_access",
        "Gym or clubhouse access.",
        "Tenant: How do I get into the estate gym?\nAssistant: Access method is in house rules — tag, code, or office registration.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "Gym access is set out in house rules (tag, PIN, or sign-up at the office). Your agent can confirm if missing.",
                "conversation_title": "Gym access",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_braai_lapa_booking",
        "Communal braai / lapa booking.",
        "Tenant: Do I need to book the lapa for Saturday?\nAssistant: Estate rules cover communal facilities — check house rules.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "Lapa or braai booking rules are in your house rules or estate app. Contact the estate manager to book if required.",
                "conversation_title": None,
                "maintenance_ticket": None,
            }
        ),
    ...
    t(
        "prop_info_recycling_bins",
        "Recycling location / day.",
        "Tenant: Where are the recycling bins and which day is recycling?\nAssistant: Estate refuse guide in house rules.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "Recycling location and collection day are in house rules or the refuse notice. Estate office can confirm.",
                "conversation_title": "Recycling info",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True, "conversation_title_non_null": True},
    ...
    t(
        "prop_info_quiet_hours",
        "Noise / quiet hours in complex.",
        "Tenant: What are the quiet hours in this estate?\nAssistant: Usually in house rules under conduct or noise.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "Quiet hours are in your house rules (noise / conduct section). If unclear, ask your property manager.",
                "conversation_title": "Quiet hours",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_storage_locker",
        "Storeroom / locker allocation.",
        "Tenant: Is there a storage locker with our unit and where is it?\nAssistant: Lease or inventory may list it — house rules for common storage.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "Check your lease, ingoing inventory, or house rules for storage allocation. Your agent can confirm the locker number and location.",
                "conversation_title": "Storage locker",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_unit_bedrooms_lease",
        "How many bedrooms / floor plan.",
        "Tenant: The ad said 3 beds but I only count 2 — what does the lease say?\nAssistant: Signed lease and annexures are definitive.",
        ["property_info", "lease"],
        chat_raw=j(
            {
                "reply": "Your signed lease and floor plan annexure define the premises. Compare with the agent if it doesn't match what you viewed.",
                "conversation_title": "Unit layout question",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_included_services",
        "What’s included (garden, DSTV, fibre).",
        "Tenant: Is garden service included in our rent for this stand?\nAssistant: Lease schedule usually lists inclusions.",
        ["property_info", "lease"],
        chat_raw=j(
            {
                "reply": "Inclusions like garden service are in your lease and rent schedule. If not listed, confirm with your agent.",
                "conversation_title": "What's included in rent",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_municipal_vs_landlord_electricity",
        "Who bills electricity / prepaid.",
        "Tenant: Do I buy prepaid from the municipality or does the landlord send a bill?\nAssistant: Lease payment clause explains.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "How electricity is billed (prepaid meter, landlord sub-meter, or municipal account) is in your lease. Ask the agent if the meter setup is unclear.",
                "conversation_title": "Electricity billing",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_estate_speed_limit",
        "Internal roads / speed limit.",
        "Tenant: What's the speed limit inside the estate?\nAssistant: Estate traffic rules in house rules or entrance signage.",
        ["property_info"],
        expected_chat_json={"maintenance_ticket_null": True},
    )
    t(
        "prop_info_intercom_visitor",
        "How visitors enter / intercom.",
        "Tenant: How do I buzz a visitor in at the gate?\nAssistant: Intercom instructions vary — house rules or welcome letter.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "Gate and intercom steps are in your welcome pack or house rules. Estate security can show you once if you're new.",
                "conversation_title": "Visitor access",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_complex_amenities_list",
        "What facilities exist on site.",
        "Tenant: What amenities does this complex have — kids' play area?\nAssistant: Marketing may differ; house rules list official facilities.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "Official communal facilities are listed in house rules and estate notices (pool, playground, etc.). Your agent can send the current amenity list.",
                "conversation_title": "Estate amenities",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_water_tank_backup",
        "Estate water backup / loadshedding water.",
        "Tenant: Is there a JoJo or backup water when municipal cuts?\nAssistant: Estate infrastructure in rules or notice to owners.",
        ["property_info"],
        expected_chat_json={"maintenance_ticket_null": True},
    )
    t(
        "prop_info_satellite_dish_rules",
        "DSTV dish / aerial installation rules.",
        "Tenant: Can I mount a dish on the wall facing the street?\nAssistant: Aesthetics rules in house rules / body corporate.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "Satellite dish placement is usually restricted by house rules or body corporate guidelines. Get written approval before installing.",
                "conversation_title": "Satellite dish rules",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_emergency_estate_number",
        "Estate security after-hours number.",
        "Tenant: What's the estate security number for after hours?\nAssistant: Welcome pack, gate house, or house rules.",
        ["property_info"],
        chat_raw=j(
            {
                "reply": "After-hours security numbers are in your welcome pack, on the access letter, or house rules. Use SAPS 10111 for crimes in progress.",
                "conversation_title": "Security contact",
                "maintenance_ticket": None,
            }
        ),
        expect_chat={"json_ok": True, "maintenance_ticket_null": True},
    ...
    t(
        "prop_info_erf_stand_number",
        "Property / erf reference on documents.",
        "Tenant: What is the erf number for our unit for municipality forms?\nAssistant: Rates account or lease annexure.",
        ["property_info"],
        expected_chat_json={"maintenance_ticket_null": True},
    )
    ...
    extras = [
        ...
        ("prop_info_laundry_room", "Communal laundry", "Tenant: Is there a shared laundry room and is it coin-operated?\nAssistant: House rules or welcome pack describe laundry facilities.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_fibre_isp", "Fibre / internet in unit", "Tenant: Which fibre provider is pre-installed in the estate?\nAssistant: Estate fibre agreement varies — check welcome letter or agent.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_tennis_court_booking", "Tennis court", "Tenant: How do I book the tennis court?\nAssistant: Estate app or office — in house rules.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_playground_age", "Kids playground rules", "Tenant: Is the playground for under-12 only?\nAssistant: House rules for play areas.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_postal_address", "Postal address for deliveries", "Tenant: What address should I use for online shopping delivery?\nAssistant: Unit number + estate street address as on lease; gate protocol in rules.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_sectional_title_levy", "Who pays levy", "Tenant: Do tenants pay body corporate levy or only the owner?\nAssistant: Typically owner — but check lease for recovery; not legal advice.", ["property_info", "lease"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_generator_estate", "Estate backup generator", "Tenant: Does the estate generator cover all units during loadshedding?\nAssistant: Estate notice or house rules describe common-area vs unit backup.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_hoa_architectural", "Alterations approval", "Tenant: Can I paint the front door a different colour?\nAssistant: Architectural / aesthetic rules need body corporate or landlord approval.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_move_truck_access", "Moving truck / large vehicle gate", "Tenant: Can a removals truck come through the estate gate?\nAssistant: Estate access rules — book gate or time slot if required.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_domestic_quarters", "Staff quarters on property", "Tenant: Is the outside room included in our lease as domestic quarters?\nAssistant: Lease premises description and inventory define included spaces.", ["property_info", "lease"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_parking_bay_number", "Allocated bay number", "Tenant: Which parking bay is allocated to unit 12B?\nAssistant: Lease schedule, parking plan annexure, or agent allocation letter.", ["property_info"], {"maintenance_ticket_null": True}, {}),
        ("prop_info_water_restriction", "Municipal water restrictions", "Tenant: Are we on level X water restrictions here?\nAssistant: Municipality publishes restrictions; estate may add rules in notices.", ["property_info"], {"maintenance_ticket_null": True}, {}),
    ]
    ...
    payload = {
        "version": 3,
        "description": "Tenant AI training & regression: chat JSON (maintenance_ticket), maintenance-draft JSON, parser edge cases, "
        "and property_info FAQs (estate rules, amenities, lease facts — no ticket). "
        "Re-run: python backend/apps/ai/fixtures/generate_tenant_ai_training_cases.py",
        "endpoints": {
