...
class ESigningAuditEventModelTests(TremlyAPITestCase):
    ...
    def test_event_types_cover_ecta_requirements(self):
        ...
        Additional event types (e.g. draft_saved, supporting_doc_*) are permitted.
        """
        required_types = {
            "link_created",
            "document_viewed",
        ...
        # Required ECTA types must all be present; extra types are fine
        self.assertTrue(
            required_types.issubset(actual_types),
            f"Missing ECTA event types: {required_types - actual_types}",
        )
