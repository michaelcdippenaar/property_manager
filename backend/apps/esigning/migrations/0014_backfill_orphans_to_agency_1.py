"""
Phase 4 — Backfill orphan rows in the esigning app to Klikk agency #1.
...
def backfill(apps, schema_editor):
    ...
    if klikk is None:
        print("[esigning.0014] no Klikk agency — skipping")
        return

    counts: dict[str, int] = {}
    skipped: list[str] = []
    for label in ESIGNING_MODELS:
        try:
            Model = apps.get_model("esigning", label)
        except LookupError:
    ...
    print(f"[esigning.0014] backfilled to agency #{klikk.pk}: {counts}")
    if skipped:
        print(f"[esigning.0014] skipped: {skipped}")
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("esigning", "0013_backfill_agency_id_esigning"),
