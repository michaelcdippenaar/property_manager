...
class AILeaseAgentRun(models.Model):
    """Per-request audit row for the Lease AI multi-agent loop.

    Persisted by `apps.leases.agent_runner.LeaseAgentRunner.finalize()` at the
    end of each `/ai-chat-v2/` request. Owns budget telemetry (call count,
    ...
    P0-7 / POPIA s14+s24 accountability fields (Wave 1):
    ``template``, ``created_by``, ``agency``, ``user_message``,
    ``document_html_before``, ``document_html_after`` were added to close the
    accountability gap flagged in the code-reviewer audit.
    """

    class TerminatedReason(models.TextChoices):
        COMPLETED = "completed", "Completed"
        CAP_CALLS = "cap_calls", "Hit LLM-call cap"
        CAP_WALLTIME = "cap_walltime", "Hit wall-clock cap"
        CAP_COST = "cap_cost", "Hit cost cap"
        CAP_RETRIES = "cap_retries", "Hit retry cap"
        ERROR = "error", "Unhandled error"

    # Correlation key for the request. Generated by the view (uuid4 or
    # x-request-id header). NOT unique — the same request_id may appear
    # multiple times on retry. Use (request_id, started_at) for idempotency
    # lookups. (P0-10: unique=True dropped to prevent IntegrityError on retry.)
    request_id = models.CharField(max_length=64, db_index=True)

    # FK-shaped but stored as a plain integer to avoid an import cycle with
    # apps.leases.Lease. Nullable because some intents (e.g. answer / Q&A
    # against a template only) have no Lease.
    lease_id = models.BigIntegerField(null=True, blank=True)

    # P0-7 POPIA accountability fields — all nullable for backwards compat
    # with existing rows. Populated by the view from the authenticated request.
    template = models.ForeignKey(
    ...
    document_html_before = models.TextField(
        blank=True, default="",
        help_text="Template HTML before the AI mutation — diff baseline.",
    )
    document_html_after = models.TextField(
        blank=True, default="",
    ...
    running_cost_usd = models.DecimalField(max_digits=6, decimal_places=4, default=0)

    terminated_reason = models.CharField(
        max_length=32, choices=TerminatedReason.choices, null=True, blank=True
    )

    # Set once corpus loader exists — see Phase 1.
    corpus_version = models.CharField(max_length=64, null=True, blank=True)

    # Per-call records — [{agent, model, input_tokens, output_tokens,
    # cache_read, cache_created, cost_usd, duration_ms, timestamp}, ...].
    # JSONField for v1; promote to a child table only if we need joins.
    ...
    class Meta:
        ordering = ["-started_at"]
        indexes = [
            models.Index(fields=["intent", "-started_at"], name="lease_airun_intent_idx"),
            models.Index(fields=["terminated_reason"], name="lease_airun_term_idx"),
        ]

    def __str__(self):
        return f"AILeaseAgentRun #{self.pk} ({self.intent}, {self.llm_call_count} calls)"
