"""
Chas Everitt Winelands scraper.
Website: https://www.chaseveritt.co.za
Strategy: Prop Data platform — same API pattern as Sotheby's.
Chas Everitt also has a dedicated Winelands branch page.
"""
...
AREA_MAP: dict[str, list[str]] = {
    ...
    AreaSlug.CITY_BOWL:    ["Cape Town City Centre", "Sea Point"],
    AreaSlug.ATLANTIC_SEABOARD: ["Camps Bay"],
    AreaSlug.SOUTHERN_SUBURBS:  ["Constantia", "Claremont"],
...
class ChasEverittScraper(BaseScraper):
    source = SourceSlug.CHAS_EVERITT
    BASE_URL = "https://www.chaseveritt.co.za"
    ...
    def _scrape_via_html(self, suburb: str, listing_type: str, area: str) -> list[dict]:
        ...
        while page <= self.MAX_PAGES:
            ...
            try:
                ...
                cards = (
                    soup.select(".listing-card, .property-card")
                    or soup.select("[class*='listing'][class*='card']")
                    or soup.select("article.property, .prop-item")
                )
                if not cards:
                    break
                for card in cards:
    ...
    def _parse_listing(self, item: dict, listing_type: str, area: str) -> dict | None:
        ...
        agency = {
            "source_agent_id": f"ce_{agent_id or listing_id}",
            "name": agent.get("name") or agent.get("fullName") or "Chas Everitt",
            "agency_name": "Chas Everitt International",
            "phone": agent.get("phone") or agent.get("cell") or "",
            "photo_url": agent.get("photo") or agent.get("image") or "",
            "agency_logo_url": f"{self.BASE_URL}/images/chas-everitt-logo.png",
        ...
        return {
            "source": SourceSlug.CHAS_EVERITT,
            "source_listing_id": f"ce_{listing_id}",
